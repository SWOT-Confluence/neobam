...
transformed data {
 ...
 vector[ntot] stationvec2;
  ...
  stationvec2 = ragged_vec(stationvec, hasdat);
...
transformed parameters {
  ...
  // calculate Q from params and obs
  vector[ntot] logQ_transform[1];
  ...
  //zo_transform is ntot
   Zo_transform[1] = Zo1 - Sobsvec.*stationvec2;
   

 logbeta[1] = log(Hobsvec -  Zo_transform[1]).*(  rep_vector(1.66,ntot) + (  rep_vector(1,ntot)./ragged_col(r[1],hasdat) )  );


logQ_transform[1]= (0.5*logSobsvec) +
                     (logbeta[1]) -
...
model {
  ...
  //this one then gets location repeated by the variable transform to logQ_S
  ...
  // in this case, we say what is the likelihood of the Q transform, which is forced to follow the flow law
  // given a sample from the Q prior? that is, try different Q vectors (starting from the prior) and see
  //how likely they match the flow-law-forced Q, which only exists with observations
  logQ_transform[1] ~ normal(ragged_col(logQ_hat,hasdat),ragged_col(logQ_sd,hasdat));
